...
let player_hand:card[]=[]
let dealer_hand:card[]=[]
let hidden_card:card
...
let kasa:HTMLParagraphElement=<HTMLParagraphElement>document.getElementById("kasa")
let can_bet=true
let message:HTMLHeadingElement=<HTMLHeadingElement>document.getElementById("message") 
...
function display_card(thing:card, id:string){
    let div:HTMLDivElement = <HTMLDivElement>document.getElementById(id)
    let image = document.createElement("img")
    image.src="karty/"+thing.figure+thing.color+".png"
    div.appendChild(image)
}
function count_cards(cards:card[]){
    let aces=0
    let sum=0
